import { jsx as t, Fragment as Pe, jsxs as a } from "react/jsx-runtime";
import { useChatLauncher as kr, useAppApi as wr, useNavigate as Er } from "@kirocrew/app-sdk";
import { PageHeader as Or, StatCard as Gt } from "@kirocrew/app-sdk/ui";
import { useState as w, useCallback as se, useEffect as Oe, useMemo as Ce, useRef as je } from "react";
...
function Mr(e, r = 3) {
  ...
  return s.reduce((l, p, u) => {
    if (u === 0) return p;
    const i = s[u - 1];
    return Lr.test(p) || Ir.test(i) ? l + p : l + " " + p;
  }, "");
}
function sr(e, r = !1) {
...
function zr(e) {
  ...
  const o = [...r].sort((p, u) => (Number(p == null ? void 0 : p.seq) || 0) - (Number(u == null ? void 0 : u.seq) || 0)), s = o.map((p) => String((p == null ? void 0 : p.note) || "")).filter(Boolean).join(" · ").slice(-512);
  if (!s) return null;
...
const Pr = /* @__PURE__ */ new Set(["done", "advanced"]), Wr = /* @__PURE__ */ new Set([
  ...
  "waived",
  "omitted"
]), Xe = (e) => !!e && typeof e == "object" && !Array.isArray(e), H = (e) => Xe(e) ? e : {}, xe = (e) => Array.isArray(e) ? e : e == null ? [] : [e], P = (...e) => e.find((r) => r != null && r !== "");
function rt(e) {
  if (e == null || e === "") return "unobservable";
  if (typeof e == "boolean") return e ? "yes" : "no";
  if (typeof e == "string" || typeof e == "number") return String(e);
  if (Array.isArray(e)) return e.length ? e.map(rt).join(" · ") : "none";
  if (Xe(e)) {
    const r = Object.entries(e);
    return r.length ? r.map(([o, s]) => `${o}: ${rt(s)}`).join(" · ") : "none";
  }
  return String(e);
}
function Je(e) {
  return xe(e).map((r, o) => {
    if (!Xe(r))
      return { key: `item-${o}`, title: rt(r), detail: null, status: null, level: null, ref: null, url: null };
    const s = P(
      r.title,
      r.label,
      r.name,
    ...
    return {
      ...
      title: String(s),
      detail: l == null || String(l) === String(s) ? null : rt(l),
...
function Ur(e) {
  return xe(e).filter((r) => r != null).map((r, o) => {
    const s = H(r), l = Xe(r) ? P(s.url, s.path, s.ref, s.id) : String(r), p = Xe(r) ? P(s.label, s.name, s.kind, s.id, s.path, s.ref, `artifact ${o + 1}`) : String(r), u = P(s.url, typeof l == "string" && /^https?:\/\//.test(l) ? l : void 0), i = P(s.preview, s.summary, s.description, s.evidence, s.detail);
    return {
      key: String(P(s.id, s.path, s.ref, `artifact-${o}`)),
      label: String(p),
      ref: l == null ? null : String(l),
      url: typeof u == "string" && /^https?:\/\//.test(u) ? u : null,
      preview: i == null ? null : rt(i),
...
function Fr(e, r) {
  const o = H(e == null ? void 0 : e.execution_envelope);
  return o.step === r ? o : xe(e == null ? void 0 : e.execution_envelope_history).map(H).reverse().find((s) => s.step === r) || {};
}
function _r(e) {
...
function lr(e, r) {
  const o = xe(e.validation_and_evidence).map(H);
  return xe(r).map(String).filter((s) => !o.some((l) => {
    const p = String(P(l.kind, l.type, l.id, "")).toLowerCase(), u = String(P(l.status, "")).toLowerCase();
    return (p === s.toLowerCase() || xe(l.satisfies).map(String).includes(s)) && Nr.has(u) && _r(l);
  }));
}
function Hr(e, r) {
  const o = xe(e.findings).map(H);
  if (!o.length) return !1;
  if (!r) return !0;
  const s = xe(P(e.sources, e.consulted_sources)).map(H).filter((p) => typeof p.url == "string" && /^https?:\/\//.test(p.url) && p.title && p.accessed_at && P(p.source_type, p.type)), l = new Set(s.flatMap((p) => [p.id && String(p.id), p.url]).filter(Boolean));
...
function Kr(e, r, o, s) {
  const l = H(e == null ? void 0 : e.intent_integrity), p = l.status === "violation" ? [`intent integrity (${xe(l.violations).join(", ")})`] : [], u = Fr(e, r), i = xe(H(u.observations).controls_runtime);
  ...
  const v = xe(e == null ? void 0 : e.decisions).map(H).filter((y) => y.step && y.step !== r || y.envelope_id && y.envelope_id !== u.id ? !1 : y.question || [
    "intent-fidelity",
    "scope-drift",
    "technical-fork",
    "capability-gap",
  ...
  for (const y of [...xe(b.required_outcome_ids), ...xe(b.hard_constraint_ids)]) {
    ...
    (!Nr.has(S) || !he.some(_r)) && c.push(`required intent coverage ${y}`);
  }
  ...
  const L = lr(s, b.evidence), A = lr(s, b.validation);
  x.evidence === "required" ? c.push(...L.map((y) => `required evidence ${y.toLowerCase()}`)) : x.evidence === "preferred" && g.push(...L.map((y) => `preferred evidence ${y.toLowerCase()}`)), x.validation === "required" ? c.push(...A.map((y) => `required validation ${y.toLowerCase()}`)) : x.validation === "preferred" && g.push(...A.map((y) => `preferred validation ${y.toLowerCase()}`));
  const I = H(u.research_policy), C = H(e == null ? void 0 : e.research_artifacts)[r], B = xe(P(s.research_and_citations, C)).map(H), X = B.filter((y) => Hr(
    y,
...
function Vr(e, r, o) {
  const s = H(e.runtime_handshakes), l = H(e.runtime_handshake), p = H(s[r] || (l.step == null || l.step === r ? l : {})), u = H(p.assignment), i = H(p.capabilities), c = H(i.tools), g = H(i.skills), v = H(p.routing), E = H(v.model), f = H(v.reasoning_effort), b = H(p.scope), x = H(b.worktree), k = H(o.routing_and_provenance), D = H(k.model), L = H(k.reasoning_effort), A = H(k.assignment), I = P(c.profile_declared, c.declared, k.declared_tools), C = P(c.actual, k.actual_tools), B = P(g.profile_declared, g.declared, k.declared_skills), X = P(g.actual, k.actual_skills);
  return {
    assignedProfile: P(
      A.assigned_profile,
      k.assigned_profile,
    ...
    effort: {
      requested: P(L.requested, k.requested_effort, f.requested) ?? null,
      applied: P(L.applied, k.applied_effort, f.applied) ?? null,
      status: P(
        L.status,
        k.effort_resolution_status,
        f.status,
        P(L.applied, k.applied_effort, f.applied) != null ? "observed" : "unobservable"
      )
    },
    tools: {
...
function Xr(e, r) {
  const o = H(e == null ? void 0 : e.gate_review), s = H(o.bundle), l = P(o.gate, e == null ? void 0 : e.stage), p = P(o.producer_step, r), u = H(e == null ? void 0 : e.step_sessions), i = Number.isInteger(o.result_revision) ? o.result_revision : null, c = P(o.status, "unobservable"), g = p ? H(e == null ? void 0 : e.step_status)[p] : void 0, v = Ur(s.artifacts), E = H(s.card_topology), f = Gr(E), b = P(E.action, "unobservable"), x = ["fan-in", "unify"].includes(String(b).toLowerCase()), k = x ? f.filter((X) => X.required && !X.complete) : [], D = [];
  (!(e != null && e.gate_review) || !Xe(e.gate_review)) && D.push("result bundle record"), (!o.bundle || !Xe(o.bundle)) && D.push("declared result bundle"), p || D.push("producer binding"), i === null && D.push("result revision"), l && (e != null && e.stage) && l !== e.stage && D.push("gate binding matches current stage"), c !== "awaiting-review" && D.push(`review status awaiting-review (currently ${c})`), Pr.has(String(g || "").toLowerCase()) || D.push(`terminal producer status (currently ${g || "unobservable"})`), P(s.summary) || D.push("result summary"), v.length === 0 && D.push("referenced artifact");
  const L = v.filter((X) => !X.ref);
  L.length > 0 && D.push(`artifact reference (${L.length} missing)`), x && f.length === 0 && D.push("declared fan-in child set"), k.length > 0 && D.push(`required child fan-in (${k.length} incomplete)`);
  const A = Kr(e, p, o, s);
  D.push(...A.missing);
  const I = xe(e == null ? void 0 : e.decisions).filter((X) => {
    const y = H(X);
  ...
  return {
    ...
    producerSessionRef: P(
      ...
      p && Xe(u[p]) ? `step_sessions.${p}` : void 0
    ) || null,
    ...
    changes: Je(s.changes_since_prior),
    artifacts: v,
    coverage: Je(s.intent_and_requirement_coverage),
    alternatives: Je(s.alternatives),
    research: Je(P(
      s.research_and_citations,
      p && H(e == null ? void 0 : e.research_artifacts)[p]
    )),
    ...
    routing: B,
    validation: Je(s.validation_and_evidence),
    risks: Je(s.known_risks),
    deviations: Je(s.omissions_and_deviations)
  };
}
...
function Qr(e) {
  ...
  if (new TextEncoder().encode(o).length > cr) return null;
...
async function Vt(e) {
  try {
    const r = await e(Yr), o = Qr(r);
    if (o)
      try {
        return { path: o, data: await e(o), source: "pointer" };
      } catch {
      }
  } catch {
  }
...
function Cr() {
  var F, ve;
  const e = wr(), [r, o] = w(null), [s, l] = w(!1), [p, u] = w("8765"), [i, c] = w(""), [g, v] = w(""), [E, f] = w(""), [b, x] = w(!1), [k, D] = w(!1), [L, A] = w(!0), [I, C] = w(!1), [B, X] = w(""), y = se((O) => {
...
`)), v(O.inbox_path || ""), D(!!O.autosync), f(""), x(!1);
  ...
  const [S, he] = w(null), [ye, le] = w(!1), me = se(async () => {
    try {
  ...
  }, [e]), [z, ee] = w(null), [$e, Te] = w(!1), _e = se(async () => {
    try {
  ...
  }, [e]), U = Ce(() => ta(i), [i]), te = Number(p), we = typeof TextEncoder > "u" ? E.length : new TextEncoder().encode(E).length, qe = !!(r != null && r.secret_configured) || we >= 32, ne = Number.isInteger(te) && te >= 1024 && te <= 65535 && (!s || U.length > 0 && qe) && (!g.trim() || g.trim().startsWith("/")), Z = async () => {
    if (!(!(r != null && r.editable) || !ne)) {
      C(!0), X("");
      try {
        const O = {
          enabled: s,
        ...
        E && (O.secret = E), y(await e.post("/apps/dlc-yolo/api/webhook/config", O));
      } catch (O) {
        X(At(O));
      } finally {
        C(!1);
    ...
    "section",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ a("div", { className: "px-4 py-4 flex flex-col gap-4", children: [
          L ? /* @__PURE__ */ t("div", { className: "text-[12px]", style: { color: "var(--muted)" }, children: "Loading receiver configuration…" }) : r && /* @__PURE__ */ a(Pe, { children: [
            ...
            !s && r.secret_configured && r.editable && /* @__PURE__ */ a("label", { className: "flex items-center gap-2 text-[11px] cursor-pointer", style: { color: "var(--muted)" }, children: [
              /* @__PURE__ */ t("input", { type: "checkbox", checked: b, onChange: (O) => x(O.target.checked) }),
              ...
              "div",
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ a("div", { children: [
                    ...
                    /* @__PURE__ */ t("div", { style: { color: "var(--text)" }, children: ((F = r.inbox) == null ? void 0 : F.pending) ?? "—" })
                  ] }),
                  /* @__PURE__ */ a("div", { children: [
                    /* @__PURE__ */ t("span", { style: { color: "var(--muted)" }, children: "Processed" }),
                    /* @__PURE__ */ t("div", { style: { color: "var(--text)" }, children: ((ve = r.inbox) == null ? void 0 : ve.processed) ?? "—" })
                  ] })
                ]
              }
            ),
              ...
              "div",
              {
                ...
                children: [
                  ...
                  S && !S.installed && /* @__PURE__ */ a("div", { className: "rounded px-2 py-1.5 text-[11px]", style: { color: "var(--warn)", border: "1px solid color-mix(in srgb, var(--warn) 40%, var(--border))" }, children: [
                    ...
                    S.install_hint && /* @__PURE__ */ t("pre", { className: "mt-1 whitespace-pre-wrap font-mono text-[10px]", style: { color: "var(--text)" }, children: S.install_hint })
                  ] }),
                  ...
                  S && S.installed && !S.running && S.receiver_ready === !1 && /* @__PURE__ */ a("div", { className: "rounded px-2 py-1.5 text-[11px]", style: { color: "var(--warn)", border: "1px solid color-mix(in srgb, var(--warn) 40%, var(--border))" }, children: [
                    ...
                    ra[S.receiver_block_reason || ""] || S.receiver_block_reason
                  ] }),
                  /* @__PURE__ */ a("div", { className: "flex gap-2", children: [
                    S != null && S.running ? /* @__PURE__ */ t(
                      "button",
                      {
                        onClick: () => void Q(),
                        disabled: ye,
                  ...
                  (S == null ? void 0 : S.autosync) && S.autosync.enabled && /* @__PURE__ */ t("div", { className: "text-[10px]", style: { color: S.autosync.error ? "var(--danger, #ef4444)" : "var(--ok)" }, children: S.autosync.error ? `Auto-sync failed: ${S.autosync.error}` : `Auto-synced ${(S.autosync.results || []).filter((O) => O.action === "updated").length} hook(s) → ${S.autosync.payload_url}` })
                ]
              }
            ),
            /* @__PURE__ */ a(
              "div",
              {
                ...
                children: [
                  ...
                  z && z.available && z.jobs.length > 0 && /* @__PURE__ */ t("div", { className: "flex flex-col gap-1", children: z.jobs.map((O) => /* @__PURE__ */ a(
                    "div",
                    {
          ...
          B && /* @__PURE__ */ t("div", { className: "rounded-md px-3 py-2 text-[11px]", style: { color: "var(--danger, #ef4444)", border: "1px solid color-mix(in srgb, var(--danger, #ef4444) 45%, var(--border))" }, children: B })
        ] }),
        /* @__PURE__ */ a("footer", { className: "px-4 py-3 flex justify-between gap-2", style: { borderTop: "1px solid var(--border)", background: "var(--card)" }, children: [
          /* @__PURE__ */ t("button", { onClick: () => void J(), disabled: L || I, className: "text-xs px-3 py-1.5 rounded-md font-medium disabled:opacity-50", style: { color: "var(--muted)" }, children: "Refresh status" }),
...
function aa({ repos: e, selectedRepos: r, onNewPipeline: o, onConfigure: s, onOpenAgents: l }) {
  const { openChat: p } = kr(), u = r.length === 1 ? r[0] : e.length === 1 ? e[0] : "", i = "/dlc-yolo", c = "text-[10px] leading-none px-1.5 py-1 rounded font-semibold";
  return /* @__PURE__ */ t(Pe, { children: /* @__PURE__ */ a("div", { "data-dlc-command-controls": !0, className: "mb-4 flex min-h-6 items-center justify-end gap-1.5 flex-wrap", children: [
    /* @__PURE__ */ t(
      "button",
      {
        onClick: () => p({ message: i }),
...
function na({ budget: e, depth: r, onSave: o }) {
  const [s, l] = w(!1), [p, u] = w(jt(e)), [i, c] = w(
    jt(e) === "custom" ? { ...e } : { ...Ft[r] || Ft.standard }
  ), g = () => {
    const f = jt(e);
    u(f), c(f === "custom" ? { ...e } : { ...Ft[r] || Ft.standard }), l(!0);
  }, v = () => {
  ...
  }, E = jt(e) === "depth" ? "budget: depth" : jt(e) === "unlimited" ? "budget: unlimited" : "budget: custom";
    ...
    s && /* @__PURE__ */ a(
      "div",
      {
        ...
        children: [
          ...
          p === "unlimited" && /* @__PURE__ */ t("div", { className: "text-[10px]", style: { color: "var(--warn)" }, children: "Literal unlimited child/effort caps · XL · proactive addenda." }),
          ...
          /* @__PURE__ */ a("div", { className: "flex justify-end gap-2 mt-1", children: [
            ...
            /* @__PURE__ */ t("button", { type: "button", onClick: v, className: "text-[10px] px-2 py-1 rounded font-semibold", style: { background: "var(--accent)", color: "var(--bg)" }, children: "Save budget" })
          ] })
        ]
      }
    )
...
function sa(e) {
  ...
  return typeof s == "string" && o[s] && typeof o[s] == "object" ? o[s] : Object.values(o).find((l) => l && typeof l == "object" && l.step === e.stage) || null;
}
function hr(e, r) {
  const o = e == null ? void 0 : e[r], s = o && typeof o == "object" ? o[e.stage] : null;
  return typeof s == "string" && s.trim() ? s.trim() : null;
}
function vr(e, { isGate: r = !1, liveObserved: o = !1 } = {}) {
  const s = typeof (e == null ? void 0 : e.stage) == "string" ? e.stage : "", l = typeof (e == null ? void 0 : e.lifecycle) == "string" ? e.lifecycle.toLowerCase() : "", p = e != null && e.step_status && typeof e.step_status == "object" ? String(e.step_status[s] || "") : "", u = sa(e), i = typeof (u == null ? void 0 : u.status) == "string" ? u.status : "", c = e != null && e.step_sessions && typeof e.step_sessions == "object" ? e.step_sessions[s] : null, g = mr.has(l) || i === "cancelling" || (c == null ? void 0 : c.writes_allowed) === !1 || !!(c != null && c.cancel_requested_at), v = s === "done" || oa.has(l) || ["completed", "cancelled", "superseded"].includes(i);
  let E, f = null;
  return v ? (E = "terminal", f = i === "cancelled" || mr.has(l) ? `terminal ${l || i}` : l || i || s || null) : g ? (E = "cancelling", f = "writes revoked; awaiting terminal observation") : p === "blocked" || i === "blocked" ? (E = "blocked", f = hr(e, "block_reason") || ((u == null ? void 0 : u.wait_reasons) || [])[0] || "step blocked") : p === "error" || i === "failed" ? (E = "error", f = hr(e, "error_reason") || (u == null ? void 0 : u.dispatch_error) || "step error") : r || i === "gate-wait" ? E = "waiting-gate" : o ? E = "running-observed" : p === "pending" || i === "running" ? (E = "pending-unconfirmed", f = "no current live observation") : ["queued", "dependency-wait", "permit-wait"].includes(i) ? (E = "queued", f = Array.isArray(u == null ? void 0 : u.wait_reasons) ? u.wait_reasons.join(" · ") : null) : i === "ready" ? E = "ready" : E = "idle", { kind: E, reason: f, ...Sr[E] };
}
...
const ia = /^\[([a-z0-9-]+)\s*[·.]\s*f?\d+\]\s*(.*)$/i;
function $r(e) {
  const r = ia.exec(String(e || ""));
...
function ca(e, r) {
  ...
  for (const u of ((p = e.topology) == null ? void 0 : p.children) || []) {
...
function ua(e, r) {
  var u;
  const o = e.id, s = ((u = e.execution_schedule) == null ? void 0 : u.nodes) || {};
  for (const [i, c] of Object.entries(s)) {
    if (!c || typeof c != "object") continue;
    const g = Ee(c.terminal_at) || Ee(c.session_at) || Ee(c.ready_at) || Ee(c.created_at);
    r({
      id: `sched:${i}`,
      at: g,
      actor: "step-agent",
  ...
  for (const i of e.event_outbox || [])
    !i || typeof i != "object" || r({
      ...
      detail: i.observed_status ? `observed: ${i.observed_status}` : i.run_id ? `run ${pa(i.run_id)}` : ""
    });
  ...
  const p = e.orchestrator_trigger;
...
function ma(e, r) {
  for (const o of (e == null ? void 0 : e.github_webhook_history) || [])
    !o || typeof o != "object" || r({
      id: `wh:${o.delivery_id}`,
      at: Ee(o.at) || Ee(o.received_at) || Ee(o.time),
      actor: "webhook",
...
function ha(e, r, o) {
  const s = e == null ? void 0 : e.id, l = (r || []).filter((g) => {
    var v;
    return g && (g.pipeline_id === s || !g.pipeline_id && ((v = g.source) == null ? void 0 : v.repo) === (e == null ? void 0 : e.repo));
  }), p = [], u = (g) => {
    g && g.at && p.push({ glyph: Tr[g.actor] || "•", ...g });
  };
  for (const g of l) ua(g, u);
  ma(o, u);
  const i = Object.fromEntries(gr.map((g, v) => [g, v]));
  p.sort((g, v) => (g.at < v.at ? -1 : g.at > v.at ? 1 : 0) || (i[g.actor] ?? 9) - (i[v.actor] ?? 9) || (g.id < v.id ? -1 : g.id > v.id ? 1 : 0));
  const c = gr.filter((g) => p.some((v) => v.actor === g));
...
function ba(e, r) {
  if (!va.includes(e)) return { ok: !1, error: `unknown request kind: ${e}` };
  const o = nr[e], s = String(r || "").trim();
  return o.reasonRequired && !s ? { ok: !1, error: "a reason is required for this request" } : s.length > ar ? { ok: !1, error: `reason exceeds ${ar} chars` } : { ok: !0 };
}
function xa({ id: e, kind: r, text: o, card: s, now: l, boundary: p }) {
  const u = ba(r, o);
...
function ya(e, r) {
  const o = Object.fromEntries(Rr.map((p) => [p, 0])), s = r && typeof r == "object" ? r : {};
...
function Ht(e) {
  ...
  const r = !!e.authority_active, o = String(e.parity_status || ""), s = o === "verified" || e.verified === !0;
  return {
    available: !0,
    authority_active: r,
    verified: s,
...
function ka({ profiles: e, initial: r, onSave: o, onClose: s }) {
  var I;
  const l = r ? "update" : "create", [p, u] = w((r == null ? void 0 : r.name) || ""), [i, c] = w((r == null ? void 0 : r.kiroAgent) || ((I = e.find((C) => C.status === "loaded")) == null ? void 0 : I.name) || ""), [g, v] = w((r == null ? void 0 : r.workspace) || ""), [E, f] = w((r == null ? void 0 : r.memoryStore) || ""), [b, x] = w(!1), [k, D] = w(""), L = br.test(p.trim()) && br.test(i.trim()) && new TextEncoder().encode(g.trim()).length <= 256 && new TextEncoder().encode(E.trim()).length <= 256, A = async () => {
    if (!(!L || b)) {
      x(!0), D("");
      try {
        await o({
          ...
          children: [
            ...
            /* @__PURE__ */ a("div", { className: "px-5 py-4 flex flex-col gap-3.5", children: [
              ...
              /* @__PURE__ */ a("label", { className: "text-[10px] uppercase tracking-wider", style: { color: "var(--muted)" }, children: [
                ...
                /* @__PURE__ */ t("datalist", { id: "dlc-agent-profile-options", children: e.map((C) => /* @__PURE__ */ t("option", { value: C.name }, C.name)) })
              ] }),
              /* @__PURE__ */ a("label", { className: "text-[10px] uppercase tracking-wider", style: { color: "var(--muted)" }, children: [
                "Workspace ",
            ...
            /* @__PURE__ */ a("footer", { className: "px-5 py-3 flex justify-end gap-2", style: { borderTop: "1px solid var(--border)", background: "var(--bg-elevated, var(--bg))" }, children: [
              /* @__PURE__ */ t("button", { onClick: s, disabled: b, className: "text-[11px] px-3 py-1.5 rounded-md disabled:opacity-40", style: { color: "var(--muted)" }, children: "Cancel" }),
              /* @__PURE__ */ t(
                "button",
                {
                  onClick: () => void A(),
...
function or({ profiles: e, crews: r, loading: o = !1, context: s, onRefresh: l, onClose: p, onSelectProfile: u, onSelectCrew: i, onSaveCrew: c }) {
  var B, X;
  const [g, v] = w("agents"), [E, f] = w(((B = e[0]) == null ? void 0 : B.name) || ""), [b, x] = w(((X = r[0]) == null ? void 0 : X.name) || ""), [k, D] = w(null);
  Oe(() => {
    var y;
    e.some((J) => J.name === E) || f(((y = e[0]) == null ? void 0 : y.name) || "");
  }, [e, E]), Oe(() => {
    var y;
    r.some((J) => J.name === b) || x(((y = r[0]) == null ? void 0 : y.name) || "");
  }, [r, b]);
  const L = e.find((y) => y.name === E), A = r.find((y) => y.name === b), I = Ce(
    () => A != null && A.kiroAgent ? e.find((y) => y.name === A.kiroAgent) : void 0,
    [A, e]
  ), C = L != null && L.prompt ? L.prompt.length > 1200 ? `${L.prompt.slice(0, 1200)}…` : L.prompt : "";
  return /* @__PURE__ */ a(
    "div",
    {
        ...
        /* @__PURE__ */ a(
          "section",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ t("div", { className: "flex min-h-0 flex-1", children: g === "agents" ? /* @__PURE__ */ a(Pe, { children: [
                ...
                /* @__PURE__ */ t("main", { className: "flex-1 min-w-0 overflow-y-auto p-5", children: L ? /* @__PURE__ */ a("div", { className: "flex flex-col gap-4", children: [
                  ...
                  /* @__PURE__ */ a("div", { className: "rounded-lg p-3 flex flex-col gap-2", style: { border: "1px solid var(--border)", background: "var(--bg-elevated, var(--bg))" }, children: [
                    /* @__PURE__ */ t(Qe, { label: "Model", value: L.model || "auto / provider default" }),
                    /* @__PURE__ */ t(Qe, { label: "Config source", value: L.sourcePath }),
                    /* @__PURE__ */ t(Qe, { label: "Prompt", value: L.prompt ? L.prompt.startsWith("file://") ? L.prompt : "inline prompt" : void 0 })
                  ] }),
                  ...
                  C && /* @__PURE__ */ a("details", { className: "rounded-lg p-3", style: { border: "1px solid var(--border)" }, children: [
                    ...
                    /* @__PURE__ */ t("pre", { className: "mt-2 text-[10px] whitespace-pre-wrap break-words max-h-56 overflow-y-auto", style: { color: "var(--muted)" }, children: C })
                  ] }),
                ...
                /* @__PURE__ */ t("main", { className: "flex-1 min-w-0 overflow-y-auto p-5", children: A ? /* @__PURE__ */ a("div", { className: "flex flex-col gap-4", children: [
                  ...
                  /* @__PURE__ */ a("div", { className: "rounded-lg p-3 flex flex-col gap-2", style: { border: "1px solid var(--border)", background: "var(--bg-elevated, var(--bg))" }, children: [
                    /* @__PURE__ */ t(Qe, { label: "kiro_agent", value: A.kiroAgent }),
                    /* @__PURE__ */ t(Qe, { label: "Workspace", value: A.workspace }),
                    /* @__PURE__ */ t(Qe, { label: "Memory store", value: A.memoryStore }),
                    /* @__PURE__ */ t(Qe, { label: "Model override", value: A.model }),
                    /* @__PURE__ */ t(Qe, { label: "Source", value: A.source })
                  ] }),
                  /* @__PURE__ */ a("section", { children: [
                  ...
                  A.kiroAgent && /* @__PURE__ */ a("div", { className: "rounded-lg p-3", style: { border: "1px solid var(--border)" }, children: [
                    ...
                    /* @__PURE__ */ a("div", { className: "flex items-center gap-2 mt-1.5", children: [
                      ...
                      /* @__PURE__ */ t("span", { className: "text-[10px]", style: { color: (I == null ? void 0 : I.status) === "loaded" ? "var(--ok)" : "var(--warn)" }, children: (I == null ? void 0 : I.status) === "loaded" ? "loaded" : "unavailable" }),
                      /* @__PURE__ */ t(
                        "button",
                        {
                          onClick: () => {
...
const Ar = Object.freeze([
  ...
  "dlcyolo-builder",
  "dlcyolo-coordinator"
]), wa = /^[A-Za-z0-9._-]{1,128}$/;
function Fe(e) {
  return typeof e == "string" && wa.test(e);
}
function Ve(e) {
...
function Na(e) {
  return !e || typeof e != "object" || Array.isArray(e) ? [] : Object.entries(e).filter(([r, o]) => Fe(r) && o && typeof o == "object" && !Array.isArray(o)).map(([r, o]) => ({
    name: r,
    kiroAgent: Fe(o.kiro_agent) ? o.kiro_agent : void 0,
    workspace: Ve(o.workspace),
    memoryStore: Ve(o.memory_store ?? o.memoryStore),
    model: Ve(o.model),
    description: Ve(o.description),
    triggers: Xt(o.triggers),
    source: Ve(o.source)
...
function _a(e, r = Ar) {
  const o = [];
  for (const l of r)
    Fe(l) && !o.includes(l) && o.push(l);
  const s = (Array.isArray(e) ? e : []).map((l) => l == null ? void 0 : l.kiroAgent).filter(Fe).sort((l, p) => l.localeCompare(p));
  for (const l of s)
    o.includes(l) || o.push(l);
  return o;
}
function Ca(e, r, o = Ar) {
  if (!Fe(e)) return;
  if (o.includes(e)) return `~/.kiro/crew/apps/dlc-yolo/agents/${e}.json`;
  const s = [...new Set(
    (Array.isArray(r) ? r : []).filter((l) => (l == null ? void 0 : l.kiroAgent) === e).map((l) => l == null ? void 0 : l.source).filter(Fe)
  )];
  if (s.length === 1)
    return `~/.kiro/agents/${s[0]}--${e}.json`;
}
function tr(e, r, o) {
  const s = Fe(r) ? r : "unknown", l = !!e && typeof e == "object" && !Array.isArray(e), p = l && Fe(e.name) ? e.name : s, u = l && e.mcpServers && typeof e.mcpServers == "object" ? Object.keys(e.mcpServers).filter(Fe) : [];
  return {
    name: p,
    description: l ? Ve(e.description) : void 0,
    prompt: l ? Ve(e.prompt) : void 0,
    model: l ? Ve(e.model) : void 0,
    tools: l ? Xt(e.tools) : [],
...
}, lt = ["manual", "assisted", "autonomous"], xt = ["quick", "standard", "deep"], Ot = { trust: "assisted", depth: "standard" }, Zt = {
  manual: "var(--info)",
  assisted: "var(--accent)",
...
function ze({ color: e, children: r, title: o, onClick: s, active: l }) {
  return /* @__PURE__ */ t(
    "button",
    {
      type: "button",
...
function Ra({ steps: e, cardsByStage: r, onNodeClick: o }) {
  const s = je(null), l = je(null), p = je(0), u = je(null), i = je(e), c = je(r), g = je([]);
  ...
  return Oe(() => {
    ...
    if (!C) return;
    const B = (J, S, he, ye, le) => {
      C.fillStyle = le, C.fillRect(J * v, S * v, he * v, ye * v);
    }, X = () => {
      ...
      for (let Q = 0; Q < S.length; Q++) {
        const z = S[Q], ee = Math.round(le * (Q + 0.5)), Te = (he[z.id] || []).length, _e = Te > 0, _ = Kt[Q % Kt.length], U = D(z), te = b - 2;
        if (me.push({ x: ee - Math.floor(le / 2), w: Math.floor(le), id: z.id }), Q < S.length - 1) {
          const ne = Math.round(le * (Q + 1.5));
          for (let Z = ee + 8; Z < ne - 8; Z += 4) B(Z, b - 1, 2, 1, "#4a3520");
        }
        if (U) {
          const ne = te - 20, Z = _e ? "#f39c12" : "#3a3222";
          B(ee - 3, ne, 6, 20, _e ? "#5c4a2a" : "#2a2418");
          for (let F = 0; F < 5; F++) B(ee - F, ne - 5 + F, F * 2 + 1, 1, Z);
          for (let F = 0; F < 5; F++) B(ee - (4 - F), ne - F, (4 - F) * 2 + 1, 1, Z);
          if (_e) {
            const F = (Math.sin(J * 0.08) + 1) / 2;
            C.globalAlpha = 0.35 + F * 0.4, B(ee - 1, ne - 6, 2, 2, "#ffd27a"), C.globalAlpha = 1;
          }
        } else {
          const ne = te - 14;
          if (B(ee - 10, ne, 20, 3, "#7a5c47"), B(ee - 10, ne - 1, 20, 1, _), B(ee - 9, ne + 3, 2, 8, "#5c4033"), B(ee + 7, ne + 3, 2, 8, "#5c4033"), B(ee - 5, ne - 9, 10, 9, "#333"), B(ee - 4, ne - 8, 8, 7, _e ? "#0a2a0a" : "#1a1a1a"), _e)
            for (let Z = 0; Z < 3; Z++) {
              const F = 2 + (J + Z * 7) % 5;
              B(ee - 3, ne - 7 + Z * 2, F, 0.8, "#33ff33");
            }
        }
        const we = Math.min(Te, 5);
        for (let ne = 0; ne < we; ne++) {
          const Z = we > 1 ? (ne - (we - 1) / 2) * 8 : 0, F = Math.round(ee + Z) - 3, ve = te - (U ? 2 : 4), O = Kt[(Q + ne) % Kt.length], ce = Math.sin(J * 0.08 + Q + ne) > 0 ? 1 : 0;
          C.fillStyle = "rgba(0,0,0,0.18)", C.fillRect(F * v, (ve + 8) * v, 6 * v, v), B(F, ve + ce, 6, 6, O), B(F + 1, ve - 4 + ce, 4, 4, "#fdd"), B(F + 1, ve - 5 + ce, 4, 1, "#333"), (J + Q * 9 + ne * 5) % 120 >= 3 && (B(F + 2, ve - 3 + ce, 1, 1, "#333"), B(F + 4, ve - 3 + ce, 1, 1, "#333")), B(F + 1, ve + 6, 1, 2, O), B(F + 4, ve + 6, 1, 2, O);
        }
        ...
        const qe = z.name.length > 12 ? z.name.slice(0, 11) + "…" : z.name;
        C.fillText(qe, ee * v, (f - 4) * v), C.textAlign = "left";
      }
      g.current = me;
  ...
  }, [x, f, b]), /* @__PURE__ */ t("div", { ref: l, className: "w-full mb-5", children: /* @__PURE__ */ t(
    "canvas",
    {
      ref: s,
      onClick: (A) => {
...
function Aa({ active: e, onChange: r, counts: o }) {
  return /* @__PURE__ */ t(
    "div",
    {
...
function tt({ rows: e, empty: r = "None recorded" }) {
...
function Ne({ label: e, value: r, status: o }) {
  return /* @__PURE__ */ a("div", { className: "min-w-0", children: [
    ...
    /* @__PURE__ */ a("div", { className: "text-[11px] mt-0.5 break-words", style: { color: rt(r) === "unobservable" ? "var(--warn)" : "var(--text)" }, children: [
      rt(r),
...
function ja({ card: e, inspection: r, producerSession: o, onClose: s, onOpenProducer: l, onApprove: p, onReject: u, onInterject: i }) {
  const c = r.routing, g = () => {
          ...
          children: [
            ...
            /* @__PURE__ */ a("div", { className: "overflow-y-auto p-4 flex flex-col gap-3", children: [
              /* @__PURE__ */ a("div", { className: "rounded-lg p-3", style: { background: r.ready ? "color-mix(in srgb, var(--ok) 8%, transparent)" : "color-mix(in srgb, var(--warn) 8%, transparent)", border: `1px solid color-mix(in srgb, ${r.ready ? "var(--ok)" : "var(--warn)"} 38%, var(--border))` }, children: [
                ...
                !r.ready && /* @__PURE__ */ t("ul", { className: "mt-1.5 pl-4 list-disc text-[10px] space-y-0.5", style: { color: "var(--muted)" }, children: r.missing.map((v) => /* @__PURE__ */ t("li", { children: v }, v)) }),
              ...
              /* @__PURE__ */ a("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3", children: [
                /* @__PURE__ */ a(Me, { title: "Result summary", children: [
                  ...
                  /* @__PURE__ */ a("div", { className: "grid grid-cols-2 gap-2 mt-3", children: [
                    /* @__PURE__ */ t(Ne, { label: "Envelope", value: r.envelopeId }),
                    /* @__PURE__ */ t(Ne, { label: "Created", value: r.createdAt })
                  ] })
                ] }),
                /* @__PURE__ */ t(Me, { title: "Changes since prior revision", children: /* @__PURE__ */ t(tt, { rows: r.changes, empty: "No revision delta recorded" }) })
              ] }),
              /* @__PURE__ */ t(Me, { title: "Artifacts and evidence references", children: r.artifacts.length ? /* @__PURE__ */ t("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-2", children: r.artifacts.map((v) => /* @__PURE__ */ a("div", { className: "rounded-md p-2", style: { background: "var(--bg-elevated, var(--card))", border: "1px solid var(--border)" }, children: [
                /* @__PURE__ */ a("div", { className: "flex gap-2 text-[11px]", children: [
                  /* @__PURE__ */ t("span", { className: "font-medium", style: { color: "var(--text)" }, children: v.label }),
                  v.kind && /* @__PURE__ */ t("span", { className: "ml-auto text-[9px]", style: { color: "var(--muted)" }, children: v.kind })
                ] }),
                v.preview && /* @__PURE__ */ t("div", { className: "mt-1 text-[10px] leading-relaxed", style: { color: "var(--muted)" }, children: v.preview }),
                v.ref && (v.url ? /* @__PURE__ */ t("a", { href: v.url, target: "_blank", rel: "noreferrer", className: "mt-1 block text-[10px] underline break-all", style: { color: "var(--accent)" }, children: v.ref }) : /* @__PURE__ */ t("code", { className: "mt-1 block text-[10px] break-all", style: { color: "var(--muted)" }, children: v.ref }))
              ] }, v.key)) }) : /* @__PURE__ */ t("div", { className: "text-[11px]", style: { color: "var(--warn)" }, children: "No referenced artifacts were published." }) }),
              /* @__PURE__ */ a("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3", children: [
                /* @__PURE__ */ t(Me, { title: "Alternatives and trade-offs", children: /* @__PURE__ */ t(tt, { rows: r.alternatives, empty: "No alternatives published" }) }),
                /* @__PURE__ */ t(Me, { title: "Research and citations", children: /* @__PURE__ */ t(tt, { rows: r.research, empty: "No research passes published" }) })
              ] }),
              /* @__PURE__ */ a("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3", children: [
                /* @__PURE__ */ t(Me, { title: "Intent and requirement coverage", children: /* @__PURE__ */ t(tt, { rows: r.coverage, empty: "No coverage records published" }) }),
                /* @__PURE__ */ t(Me, { title: "Omissions and deviations", children: /* @__PURE__ */ t(tt, { rows: r.deviations, empty: "No omissions or deviations recorded" }) })
              ] }),
              /* @__PURE__ */ a(Me, { title: "Card topology and integration", children: [
                /* @__PURE__ */ a("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-3 mb-3", children: [
                  /* @__PURE__ */ t(Ne, { label: "Action", value: r.topology.action }),
                  /* @__PURE__ */ t(Ne, { label: "Integration owner", value: r.topology.integrationOwner }),
                  /* @__PURE__ */ t(Ne, { label: "Integration status", value: r.topology.integrationStatus }),
                  /* @__PURE__ */ t(Ne, { label: "Required children incomplete", value: r.topology.incompleteRequiredChildren.length })
                ] }),
                r.topology.children.length > 0 ? /* @__PURE__ */ t("div", { className: "flex flex-col gap-1.5", children: r.topology.children.map((v) => /* @__PURE__ */ a("div", { className: "flex items-center gap-2 rounded-md px-2 py-1.5 text-[11px]", style: { background: "var(--bg-elevated, var(--card))", border: "1px solid var(--border)" }, children: [
                  /* @__PURE__ */ t("span", { style: { color: "var(--text)" }, children: v.label }),
                  /* @__PURE__ */ t("span", { className: "ml-auto text-[9px]", style: { color: v.required ? "var(--warn)" : "var(--muted)" }, children: v.required ? "required" : "optional" }),
                  /* @__PURE__ */ t("span", { className: "text-[9px]", style: { color: /done|advanced|complete|consume|integrate|waive|omit/i.test(v.status) ? "var(--ok)" : "var(--warn)" }, children: v.status })
                ] }, v.key)) }) : /* @__PURE__ */ t("div", { className: "text-[11px]", style: { color: "var(--muted)" }, children: "No child topology recorded." })
              ] }),
              /* @__PURE__ */ a("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-3", children: [
                /* @__PURE__ */ t(Me, { title: "Budget consumption", children: /* @__PURE__ */ a("div", { className: "grid grid-cols-1 gap-3", children: [
                  /* @__PURE__ */ t(Ne, { label: "Allocated", value: r.budget.allocated }),
                  /* @__PURE__ */ t(Ne, { label: "Consumed", value: r.budget.consumed }),
                  /* @__PURE__ */ t(Ne, { label: "Remaining", value: r.budget.remaining })
                ] }) }),
                /* @__PURE__ */ t(Me, { title: "Routing and runtime provenance", children: /* @__PURE__ */ a("div", { className: "grid grid-cols-2 gap-3", children: [
                  /* @__PURE__ */ t(Ne, { label: "Assigned profile", value: c.assignedProfile }),
                  /* @__PURE__ */ t(Ne, { label: "Effective profile", value: c.effectiveProfile }),
                  /* @__PURE__ */ t(Ne, { label: "Model requested", value: c.model.requested }),
                  /* @__PURE__ */ t(Ne, { label: "Model applied", value: c.model.applied, status: c.model.status }),
                  /* @__PURE__ */ t(Ne, { label: "Provider / version", value: c.model.provider || c.model.version ? [c.model.provider, c.model.version].filter(Boolean) : null }),
                  /* @__PURE__ */ t(Ne, { label: "Effort requested", value: c.effort.requested }),
                  /* @__PURE__ */ t(Ne, { label: "Effort applied", value: c.effort.applied, status: c.effort.status }),
                  /* @__PURE__ */ t(Ne, { label: "Tools available", value: c.tools.actual, status: c.tools.status }),
                  /* @__PURE__ */ t(Ne, { label: "Skills available", value: c.skills.actual, status: c.skills.status }),
                  /* @__PURE__ */ t(Ne, { label: "Network scope", value: c.network.actual, status: c.network.status }),
                  /* @__PURE__ */ t(Ne, { label: "Write scope", value: c.write.actual, status: c.write.status }),
                  /* @__PURE__ */ t(Ne, { label: "Worktree / branch", value: c.worktree })
                ] }) })
              ] }),
              /* @__PURE__ */ a("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-3", children: [
                /* @__PURE__ */ t(Me, { title: "Validation and evidence", children: /* @__PURE__ */ t(tt, { rows: r.validation, empty: "No validation results published" }) }),
                /* @__PURE__ */ t(Me, { title: "Known risks", children: /* @__PURE__ */ t(tt, { rows: r.risks, empty: "No known risks recorded" }) }),
                /* @__PURE__ */ t(Me, { title: "Open decisions and questions", children: /* @__PURE__ */ t(tt, { rows: r.decisions, empty: "No open decisions recorded" }) })
              ] })
            ] }),
...
function Ea({ card: e, openChat: r }) {
  const o = e.bootstrap, s = e.intent_contract || e.intent;
  if (!o && !s) return null;
  const l = `pipeline ${e.pipeline_id || ""} card ${e.id} (${e.title})`, p = (g, v) => /* @__PURE__ */ t(
    "button",
    {
  ...
  return /* @__PURE__ */ a("div", { className: "mt-2 pt-2", style: { borderTop: "1px dashed var(--border)" }, children: [
    ...
    /* @__PURE__ */ a("div", { className: "flex flex-col gap-1 text-[10px]", children: [
      ...
      /* @__PURE__ */ a("div", { className: "flex items-center gap-2 flex-wrap", children: [
        ...
        p(`elaborate step ${e.stage}`, "Elaborate step")
      ] }),
      /* @__PURE__ */ a("div", { className: "flex items-center gap-2 flex-wrap", children: [
...
function Oa({ cards: e, schedulerState: r, statePath: o, readAppFile: s, onClose: l }) {
  const p = Ce(() => ya(e, r), [e, r]), [u, i] = w(Ht(null)), [c, g] = w([]), [v, E] = w([]);
  Oe(() => {
    ...
    return s(x).then((D) => {
      if (!k)
        try {
          i(Ht(JSON.parse(D.content || "null")));
        } catch {
          i(Ht(null));
        }
    }).catch(() => {
    ...
    for (const k of e) {
      ...
      const L = k.worktree_lease;
      L && x.push({ card: k.id, branch: L.branch, status: L.status });
    }
          ...
          children: [
            ...
            /* @__PURE__ */ a("div", { className: "px-5 py-3 overflow-y-auto text-[11px]", children: [
              /* @__PURE__ */ a(f, { title: "Runtime / scheduler", children: [
                /* @__PURE__ */ t("div", { className: "flex flex-wrap gap-2", children: Rr.map((b) => /* @__PURE__ */ a("span", { className: "px-2 py-0.5 rounded-full", style: { background: "color-mix(in srgb, var(--accent) 10%, transparent)", color: p.counts[b] ? "var(--text)" : "var(--muted)" }, children: [
                  b,
                  " ",
                  p.counts[b]
                ] }, b)) }),
                p.waitReasons.length > 0 && /* @__PURE__ */ t("div", { className: "mt-2", children: p.waitReasons.map((b, x) => /* @__PURE__ */ a("div", { style: { color: "var(--muted)" }, children: [
                  "⛔ ",
                  b.card,
                  ": ",
                  b.reason
...
function qa({ card: e, cardStatus: r, effectiveCapability: o, onClose: s }) {
  var E, f, b;
  const [l, p] = w("overview"), u = [
    ...
    ["execution", "Execution"]
  ], i = e.execution_schedule, c = i != null && i.current_node_id ? (E = i == null ? void 0 : i.nodes) == null ? void 0 : E[i.current_node_id] : void 0, g = e.worktree_lease, v = e.topology;
  return /* @__PURE__ */ t(
    "div",
    {
      className: "fixed inset-0 z-[70] flex items-center justify-center p-4",
          ...
          children: [
            ...
            /* @__PURE__ */ a("div", { className: "px-5 py-3 overflow-y-auto text-[11px]", children: [
              l === "overview" && /* @__PURE__ */ a("div", { children: [
                (f = e.source) != null && f.url && fr(e.source.url) ? ue("source", null) || /* @__PURE__ */ a("div", { className: "text-[11px] py-0.5", children: [
                ...
                ] }) : ue("source", (b = e.source) == null ? void 0 : b.repo),
                ue("pipeline", e.pipeline_id),
                ue("workspace", e.workspace),
                ue("stage", e.stage),
                ue("lifecycle", e.lifecycle),
                ue("SoT", e.sot),
                ue("status", `${r.label}${r.reason ? ` — ${r.reason}` : ""}`),
                ue("trust", e.trust ? `${e.trust} (override)` : "inherited"),
                ue("depth", e.depth ? `${e.depth} (override)` : "inherited"),
                ue("capability", o),
                ue("effort", e.effort ? JSON.stringify(e.effort) : null),
                ue("writes_allowed", e.writes_allowed === !1 ? "false (cancel requested)" : null)
              ] }),
              ...
              l === "history" && /* @__PURE__ */ a("div", { children: [
                ...
                (e.gate_history || []).length > 0 && /* @__PURE__ */ a(Pe, { children: [
                ...
                (e.interjection || []).length > 0 && /* @__PURE__ */ a(Pe, { children: [
              ...
              l === "execution" && /* @__PURE__ */ a("div", { children: [
                ...
                ue("model (requested)", e.model_request),
                ue("model (applied)", e.model_applied),
                v && /* @__PURE__ */ a(Pe, { children: [
                ...
                ue("cancel requested", e.cancel_requested_at),
                e.writes_allowed === !1 && ue("terminal observed", "pending (cooperative cancel in progress)")
              ] })
            ] }),
  ...
  return /* @__PURE__ */ a("div", { className: "relative inline-block", children: [
    ...
    r && /* @__PURE__ */ t(
      "div",
      {
        ...
        children: Object.entries(nr).map(([l, p]) => /* @__PURE__ */ t(
          "button",
          {
...
function gt({ card: e, config: r, isGate: o, cardStatus: s, effectiveCapability: l, producerStep: p, producerSession: u, onOpenProducer: i, onApprove: c, onReject: g, onCycleTrust: v, onCycleDepth: E, onSetBudget: f, onInterject: b, onResolveDecision: x, onOpenOrchestrator: k, liveView: D, allCards: L, onOpenCard: A, onRequest: I, onOpenStepSession: C, onCancelCard: B }) {
  var O, ce, Le, at, ae, it, pe;
  const X = o ? "var(--warn)" : s.kind === "idle" ? "var(--border-strong, var(--border))" : s.color, y = e.trust || r.trust, J = e.depth || r.depth, S = ((O = e.parked) == null ? void 0 : O.length) || 0, he = Object.values(e.step_sessions || {}).some(
    ($) => !!$.last_response_at && !$.chat_disabled_at && !$.superseded && (!$.last_response_handled_at || $.last_response_handled_at < $.last_response_at)
  ), [ye, le] = w(!1), [me, Se] = w(""), [Q, z] = w(!1), [ee, $e] = w(!1), [Te, _e] = w(!1), { openChat: _ } = kr(), U = Ce(() => la(e), [e]), te = Ce(() => ca(e, L || []), [e, L]), we = Ce(() => da(e), [e]), qe = Ce(() => {
    if (!we) return null;
    const $ = (L || []).find((W) => W.id === we);
    return $ ? { id: $.id, title: $.title } : null;
  }, [we, L]), ne = U.length > 0 || te.length > 0 || !!qe, Z = Ce(
    () => o ? Xr(e, p) : null,
    [e, o, p]
  ), F = () => {
    const $ = window.prompt(`Why reject revision ${(Z == null ? void 0 : Z.revision) ?? "unknown"}?`);
    $ != null && $.trim() && g && g($.trim());
  }, ve = (e.decisions || []).filter(($) => !$.chosen && !$.resolved_at && (!!$.action || !!$.options));
  return /* @__PURE__ */ a(
    "div",
    {
      id: `card-${e.id}`,
      ...
      children: [
        ...
        ((ce = e.source) == null ? void 0 : ce.repo) && /* @__PURE__ */ a(
          "a",
          {
        ...
        (() => {
          var W;
          const $ = (W = e.step_summaries) == null ? void 0 : W[e.stage];
          return $ != null && $.headline ? /* @__PURE__ */ a("div", { className: "mt-1 flex items-start gap-1 text-[11px] leading-snug", title: $.description || $.headline, children: [
            $.needs_human ? /* @__PURE__ */ t("span", { "aria-label": "needs you", title: "Needs you", style: { color: "var(--warn)" }, children: "🔴" }) : /* @__PURE__ */ t("span", { "aria-hidden": "true", style: { color: "var(--muted)" }, children: "•" }),
            /* @__PURE__ */ t("span", { className: "truncate", style: { color: $.needs_human ? "var(--warn)" : "var(--text)" }, children: $.headline })
          ] }) : null;
        })(),
        ...
        /* @__PURE__ */ a(
          "div",
          {
            ...
            children: [
              ...
              e.lifecycle && /* @__PURE__ */ a(ze, { color: "var(--muted)", title: `card lifecycle: ${e.lifecycle}`, children: [
                "🔄 ",
                e.lifecycle
              ] }),
              S > 0 && /* @__PURE__ */ a(ze, { color: "var(--warn)", title: `${S} parked idea(s)`, children: [
                "⏸ ",
                S
              ] }),
              he && /* @__PURE__ */ t(ze, { color: "var(--accent)", active: !0, title: "A response in an enabled linked agent chat is being applied to this card", children: "↪ chat response" }),
              typeof ((Le = e.effort) == null ? void 0 : Le.total) == "number" && e.effort.total > 0 && /* @__PURE__ */ a(ze, { color: "var(--info)", title: `estimated effort: ${e.effort.total} points`, children: [
                "⚡ ",
                e.effort.total
              ] }),
              ...
              e.decisions && e.decisions.length > 0 && (() => {
                const $ = e.decisions[e.decisions.length - 1];
                return /* @__PURE__ */ a(
                  ze,
                  {
                    color: "var(--accent)",
                    title: `${e.decisions.length} decision${e.decisions.length === 1 ? "" : "s"} — last: ${$.question || $.kind || ""}${$.action ? ` → ${$.action}` : ""}${$.rationale ? `
${$.rationale}` : ""}`,
                    children: [
                      "⚖ ",
                      e.decisions.length
        ...
        o && c && g && /* @__PURE__ */ a("div", { className: "mt-2.5 flex gap-1.5 items-center flex-wrap", children: [
          (() => {
            const $ = (e.gate_commands || []).filter((He) => He.gate === e.stage), W = $.length ? $[$.length - 1] : void 0, ke = (W == null ? void 0 : W.status) === "pending", Ye = (W == null ? void 0 : W.status) === "rejected", De = (W == null ? void 0 : W.status) === "applied" || (W == null ? void 0 : W.status) === "approved";
            return /* @__PURE__ */ a(Pe, { children: [
              /* @__PURE__ */ a(
                "button",
                {
                  ...
                  children: [
                    ke && (W == null ? void 0 : W.action) === "approve" && /* @__PURE__ */ t(ft, { size: 10 }),
              ...
              /* @__PURE__ */ a(
                "button",
                {
                  ...
                  onClick: F,
                  children: [
                    ke && (W == null ? void 0 : W.action) === "reject" && /* @__PURE__ */ t(ft, { size: 10 }),
          ...
          (e.stage === "gate-review" || /review/i.test(e.stage || "")) && (() => {
            var De, He, Ie;
            const $ = (De = e.source) == null ? void 0 : De.repo;
            if (!$) return null;
            const W = (He = e.artifacts) == null ? void 0 : He.pr_url, ke = W && ((Ie = /\/pull\/(\d+)/.exec(W)) == null ? void 0 : Ie[1]), Ye = `/code-review-sage?repo=${encodeURIComponent("https://github.com/" + $)}` + (ke ? `&pr=${ke}` : "");
            return /* @__PURE__ */ a(
              "a",
              {
                href: Ye,
        ...
        D && /* @__PURE__ */ t(Fa, { live: D }),
        /* @__PURE__ */ t(Ea, { card: e, openChat: _ }),
            ...
            children: [
                ...
                /* @__PURE__ */ a("div", { className: "flex gap-1.5", children: [
                  /* @__PURE__ */ t(
                    "button",
                    {
                      ...
                      style: { background: "var(--accent)", color: "var(--bg)" },
                      onClick: () => {
                        me.trim() && (b("note", me.trim()), Se(""), le(!1));
                      },
                      children: "Send"
                    }
                  ),
              ...
              k && /* @__PURE__ */ t(
                "button",
                {
                  className: "text-[10px] hover:underline",
                  style: { color: "var(--muted)" },
                  title: (at = e.orchestrator_session) != null && at.slot_key || (ae = e.orchestrator_session) != null && ae.session_key ? "Open this pipeline’s orchestrator session" : "Trigger an inspectable orchestrator session for this card",
                  onClick: () => k(),
                  children: (it = e.orchestrator_session) != null && it.slot_key || (pe = e.orchestrator_session) != null && pe.session_key ? "⚙ open orchestrator" : "⚙ orchestrator"
                }
              ),
              ne && /* @__PURE__ */ a(
                "button",
              ...
              B && !["cancelled", "canceled", "retired", "merged"].includes(String(e.lifecycle || "")) && /* @__PURE__ */ t(
                "button",
                {
                  className: "text-[10px] hover:underline",
...
function bt({ title: e, count: r, children: o, id: s }) {
...
function Ia({ config: e, onSet: r }) {
  function o({ label: s, value: l, options: p, tokens: u, onPick: i }) {
    return /* @__PURE__ */ a("div", { className: "flex items-center gap-2", children: [
      ...
      /* @__PURE__ */ t("div", { className: "flex gap-0.5 p-0.5 rounded-md", style: { background: "var(--bg-elevated, var(--card))", border: "1px solid var(--border)" }, children: p.map((c) => {
        const g = l === c;
        return /* @__PURE__ */ t(
          "button",
          {
  ...
  return /* @__PURE__ */ a(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ t(o, { label: "Trust", value: e.trust, options: lt, tokens: Zt, onPick: (s) => r({ trust: s }) }),
        /* @__PURE__ */ t(o, { label: "Depth", value: e.depth, options: xt, tokens: Jt, onPick: (s) => r({ depth: s }) }),
...
function Da({ pipeline: e, cards: r, extras: o, onOpenCard: s }) {
  ...
  return e ? l.length === 0 ? /* @__PURE__ */ t("div", { className: "text-sm p-3", style: { color: "var(--muted)" }, children: "No recorded events for this pipeline yet." }) : /* @__PURE__ */ a("div", { className: "w-full overflow-x-auto pb-4", children: [
    ...
    /* @__PURE__ */ t("div", { className: "text-[10px] mb-2 flex flex-wrap gap-3", style: { color: "var(--muted)" }, children: p.map((i) => /* @__PURE__ */ a("span", { children: [
      Tr[i] || "•",
      " ",
      i
    ] }, i)) }),
    /* @__PURE__ */ t("ol", { className: "flex flex-col gap-1.5", style: { borderLeft: "1px solid var(--border)", paddingLeft: "10px" }, children: l.map((i) => {
      const c = Ma[i.kind] || "var(--text)";
...
function za({ repos: e, selected: r, onToggle: o, onClear: s, onAddWorkspace: l, onEdit: p }) {
  const u = e.reduce((g, v) => g + v.count, 0), i = r.size === 0, c = ({ name: g, count: v, label: E, checked: f, onClick: b, isAll: x }) => {
    const [k, D] = w(!1);
    return /* @__PURE__ */ a(
      "div",
      {
      ...
      children: [
        ...
        e.map((g) => /* @__PURE__ */ t(
          c,
          {
            name: g.name,
            count: g.count,
            label: (jr.has(g.name) ? "Example: " : "") + (g.name.includes("/") ? g.name.split("/")[1] : g.name),
...
function Wa({ initial: e, agentProfiles: r, crews: o, repo: s, stepName: l, onSave: p, onSaveCrew: u, onClose: i }) {
  const [c, g] = w(e.name || ""), [v, E] = w(e.role || ""), [f, b] = w(e.tools || ["read"]), [x, k] = w(e.model || "auto"), [D, L] = w(e.crew || ""), [A, I] = w(e.addenda || []), [C, B] = w(e.capability || ""), [X, y] = w(e.trust || ""), [J, S] = w(e.depth || ""), [he, ye] = w(!1), le = r.find((_) => _.name === c), me = o.find((_) => _.name === D), Se = [.../* @__PURE__ */ new Set([...Pa, ...f])], Q = (_) => {
    const U = $a({ name: c, role: v, tools: f, model: x, crew: D, addenda: A, capability: C, trust: X, depth: J }, _);
  ...
  }), $e = (_, U) => I((te) => te.map((we, qe) => qe === _ ? { ...we, ...U } : we)), Te = (_) => I((U) => U.filter((te, we) => we !== _)), _e = c.trim().length > 0;
  return /* @__PURE__ */ a("div", { className: "flex flex-col h-full", children: [
    ...
    /* @__PURE__ */ a("div", { className: "px-5 py-4 flex flex-col gap-3.5 flex-1 overflow-y-auto", children: [
      r.length > 0 && /* @__PURE__ */ a("div", { children: [
        ...
        /* @__PURE__ */ t("div", { className: "mt-1 flex flex-wrap gap-1.5", children: r.map((_) => /* @__PURE__ */ t(
          "button",
          {
            onClick: () => Q(_),
      ...
      /* @__PURE__ */ a("div", { children: [
        ...
        /* @__PURE__ */ t("div", { className: "mt-1 flex flex-wrap gap-1.5", children: Se.map((_) => {
          ...
          /* @__PURE__ */ a(
            "select",
            {
              ...
              children: [
                ...
                ["readonly", "authoring", "builder", "coordinator"].map((_) => /* @__PURE__ */ t("option", { value: _, children: _ }, _))
              ]
            }
          )
        ] }),
      ...
      /* @__PURE__ */ a("div", { children: [
          ...
          /* @__PURE__ */ a(
            "select",
            {
              ...
              children: [
                ...
                o.map((_) => /* @__PURE__ */ t("option", { value: _.name, children: _.name }, _.name))
              ]
            }
          )
        ] }),
        me && /* @__PURE__ */ a("div", { className: "text-[10px] mt-1 text-right", style: { color: "var(--muted)" }, children: [
          "Global route ",
          /* @__PURE__ */ t("code", { children: me.name }),
          " → ",
          /* @__PURE__ */ t("code", { children: me.kiroAgent || "profile unknown" }),
        ...
        A.map((_, U) => /* @__PURE__ */ a("div", { className: "flex items-center gap-1.5 mb-1.5", children: [
          /* @__PURE__ */ t(
            "select",
            {
              ...
              style: { background: "var(--bg-elevated, var(--bg))", border: "1px solid var(--border)", color: "var(--text)" },
              children: o.map((te) => /* @__PURE__ */ t("option", { value: te.name, children: te.name }, te.name))
            }
          ),
          /* @__PURE__ */ a(
            "select",
          ...
          /* @__PURE__ */ t("button", { onClick: () => Te(U), className: "w-5 h-5 flex items-center justify-center flex-shrink-0", style: { color: "var(--muted)" }, "aria-label": "Remove addendum", children: /* @__PURE__ */ t("svg", { width: "10", height: "10", viewBox: "0 0 12 12", children: /* @__PURE__ */ t("path", { d: "M2 2l8 8M10 2l-8 8", fill: "none", stroke: "currentColor", strokeWidth: "1.6", strokeLinecap: "round" }) }) })
        ] }, U))
      ] }),
      /* @__PURE__ */ a("div", { className: "flex items-center justify-between", children: [
        /* @__PURE__ */ t("span", { className: "text-[11px] uppercase tracking-wider", style: { color: "var(--muted)" }, children: "Trust" }),
        /* @__PURE__ */ t("div", { className: "flex gap-0.5 p-0.5 rounded-md", style: { background: "var(--bg-elevated, var(--card))", border: "1px solid var(--border)" }, children: ["", ...lt].map((_) => {
          const U = X === _;
          return /* @__PURE__ */ t(
            "button",
            {
              onClick: () => y(_),
              className: "text-[11px] px-2 py-0.5 rounded font-semibold",
              style: { color: U ? _ ? Zt[_] : "var(--text)" : "var(--muted)", background: U ? "var(--bg-hover, var(--border))" : "transparent" },
              children: _ || "inherit"
            },
            _ || "inherit"
          );
      ...
      /* @__PURE__ */ a("div", { className: "flex items-center justify-between", children: [
        ...
        /* @__PURE__ */ t("div", { className: "flex gap-0.5 p-0.5 rounded-md", style: { background: "var(--bg-elevated, var(--card))", border: "1px solid var(--border)" }, children: ["", ...xt].map((_) => {
          const U = J === _;
          return /* @__PURE__ */ t(
            "button",
            {
              onClick: () => S(_),
              className: "text-[11px] px-2 py-0.5 rounded font-semibold",
              style: { color: U ? _ ? Jt[_] : "var(--text)" : "var(--muted)", background: U ? "var(--bg-hover, var(--border))" : "transparent" },
              children: _ || "inherit"
            },
            _ || "inherit"
          );
...
function yr({ candidates: e, existingRepos: r, defaults: o, agentProfiles: s, crews: l, onCreate: p, onSaveCrew: u, onClose: i, editPipeline: c, cardCount: g, isExample: v, onDelete: E }) {
  var qt, Lt, It, Ct, St, $t, Mt, dt, pt, ut, Dt, Tt, Rt, Bt, mt;
  const f = !!c, [b, x] = w((c == null ? void 0 : c.repo) || ""), [k, D] = w((c == null ? void 0 : c.workspace) || "default"), [L, A] = w((c == null ? void 0 : c.repo_path) || ""), [I, C] = w((c == null ? void 0 : c.source) || "manual"), [B, X] = w((c == null ? void 0 : c.trust) || o.trust), [y, J] = w((c == null ? void 0 : c.depth) || o.depth), S = c == null ? void 0 : c.budget, [he, ye] = w(
    S ? S.max_child_cards === "unlimited" && S.effort_ceiling === "unlimited" ? "unlimited" : "custom" : "depth"
  ), [le, me] = w(
    () => S && S.max_child_cards !== "unlimited" && S.effort_ceiling !== "unlimited" ? { ...S } : xr((c == null ? void 0 : c.depth) || o.depth)
...
`)), [U, te] = w((c == null ? void 0 : c.self_enabling) ?? !1), [we, qe] = w((c == null ? void 0 : c.approach) || "simplified"), [ne, Z] = w((c == null ? void 0 : c.sync_mode) || "poll"), [F, ve] = w(() => {
    var h;
    return (h = c == null ? void 0 : c.steps) != null && h.length ? c.steps.map((T) => ({ ...T })) : Yt.map((T) => ({ ...T }));
  }), [O, ce] = w(null), [Le, at] = w(""), [ae, it] = w("settings"), [pe, $] = w(!1), W = (h) => h.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "step", ke = (h, T) => ve((ge) => ge.map((be, de) => de === h ? { ...be, ...T } : be)), Ye = (h) => ve((T) => T.filter((ge, be) => be !== h)), De = (h, T) => ve((ge) => {
    const be = h + T;
  ...
  ).values()], fe = ct.every((h) => /^[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?$/.test(h)), kt = /^[A-Za-z0-9_.-]{1,80}$/.test(k), wt = (/^[^/\s]+\/[^/\s]+$/.test(Ze(b)) || e.some((h) => h.repo && h.repo === b)) && fe && kt, Nt = !f && r.has(Ze(b)), _t = ({ value: h, options: T, tokens: ge, onPick: be }) => /* @__PURE__ */ t("div", { className: "flex gap-0.5 p-0.5 rounded-md", style: { background: "var(--bg-elevated, var(--card))", border: "1px solid var(--border)" }, children: T.map((de) => {
    const Ke = h === de;
    return /* @__PURE__ */ t(
      "button",
      {
        ...
        style: {
          color: Ke ? ge[de] : "var(--muted)",
          background: Ke ? `color-mix(in srgb, ${ge[de]} 16%, transparent)` : "transparent",
          boxShadow: Ke ? `inset 0 0 0 1px color-mix(in srgb, ${ge[de]} 45%, transparent)` : "none"
        },
        children: de
      },
      de
    );
  }) }), nt = { "issue-radar": [], workspace: [], manual: [] };
  e.forEach((h) => {
    var T;
    (nt[T = h.source] || (nt[T] = [])).push(h);
  });
        ...
        "div",
        {
          ...
          children: [
            ...
            O !== null ? /* @__PURE__ */ t(
              ...
              {
                initial: {
                  name: ((Lt = (qt = F[O]) == null ? void 0 : qt.agent) == null ? void 0 : Lt.name) || "",
                  role: (Ct = (It = F[O]) == null ? void 0 : It.agent) == null ? void 0 : Ct.role,
                  tools: ($t = (St = F[O]) == null ? void 0 : St.agent) == null ? void 0 : $t.tools,
                  model: (dt = (Mt = F[O]) == null ? void 0 : Mt.agent) == null ? void 0 : dt.model,
                  crew: (ut = (pt = F[O]) == null ? void 0 : pt.agent) == null ? void 0 : ut.crew,
                  addenda: (Dt = F[O]) == null ? void 0 : Dt.addenda,
                  capability: (Tt = F[O]) == null ? void 0 : Tt.capability,
                  trust: (Rt = F[O]) == null ? void 0 : Rt.trust,
                  depth: (Bt = F[O]) == null ? void 0 : Bt.depth
                },
                agentProfiles: s,
                crews: l,
                repo: b,
                stepName: ((mt = F[O]) == null ? void 0 : mt.name) || "",
                onSaveCrew: u,
                onClose: () => ce(null),
                onSave: (h) => {
                  ke(O, {
              ...
              /* @__PURE__ */ a("div", { className: "px-5 py-4 flex items-center justify-between", style: { borderBottom: "1px solid var(--border)" }, children: [
                /* @__PURE__ */ a("div", { children: [
                  ...
                  /* @__PURE__ */ t("div", { className: "text-xs mt-0.5", style: { color: "var(--muted)" }, children: f ? b.includes("/") ? b.split("/")[1] : b : "Configure a pipeline for a repository or workspace" })
                ] }),
                /* @__PURE__ */ t("button", { onClick: i, className: "text-lg leading-none px-2", style: { color: "var(--muted)" }, children: "×" })
              ] }),
              /* @__PURE__ */ t("div", { className: "px-5 pt-3 flex gap-1", style: { borderBottom: "1px solid var(--border)" }, children: er.map((h) => {
                ...
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ a("div", { children: [
                      ...
                      !f && b && Ze(b) !== b && /* @__PURE__ */ a("div", { className: "text-[11px] mt-1", style: { color: "var(--muted)" }, children: [
                        "→ ",
                        /* @__PURE__ */ t("code", { style: { color: "var(--accent)" }, children: Ze(b) })
                      ] }),
                      Nt && /* @__PURE__ */ t("div", { className: "text-[11px] mt-1", style: { color: "var(--danger)" }, children: "A pipeline for this repo already exists." }),
                      /* @__PURE__ */ t("div", { className: "mt-2 flex flex-col gap-2", children: ["issue-radar", "workspace"].map((h) => nt[h].length > 0 && /* @__PURE__ */ a("div", { children: [
                        /* @__PURE__ */ t("div", { className: "text-[10px] uppercase tracking-wider mb-1", style: { color: "var(--muted)" }, children: Qt[h] }),
                        /* @__PURE__ */ t("div", { className: "flex flex-wrap gap-1.5", children: nt[h].map((T) => {
                          const ge = `${h}:${T.workspace || T.repo}:${T.path || ""}`, be = T.source === "workspace" ? k === T.workspace && L === (T.path || "") : b === T.repo;
                          return /* @__PURE__ */ t(
                            "button",
                            {
                              onClick: () => Ie(T),
                    ...
                    /* @__PURE__ */ a("div", { className: "flex items-center justify-between", children: [
                      /* @__PURE__ */ a("div", { className: "min-w-0 pr-3", children: [
                        ...
                        /* @__PURE__ */ t("div", { className: "text-[11px]", style: { color: "var(--muted)" }, children: ne === "webhook" ? "Webhook is the fast path; the safety-net poll reconciles this pipeline on a longer window. Requires the app-wide webhook receiver enabled — falls back to polling if it is not." : "Poll reconciles this pipeline every cycle (default). Correct when no webhook is configured." })
                      ] }),
                      /* @__PURE__ */ t("div", { className: "flex rounded-md overflow-hidden flex-shrink-0", style: { border: "1px solid var(--border)" }, children: ["poll", "webhook"].map((h) => /* @__PURE__ */ t(
                        "button",
                        {
                          ...
                          style: {
                            background: ne === h ? "var(--accent)" : "transparent",
                            color: ne === h ? "var(--bg)" : "var(--muted)"
                          },
                    ...
                    /* @__PURE__ */ a("div", { children: [
                      ...
                      /* @__PURE__ */ t("div", { className: "flex flex-col gap-1.5", children: F.map((h, T) => {
                        var ge, be;
                        return /* @__PURE__ */ a(
                          "div",
                          {
                            ...
                            children: [
                              /* @__PURE__ */ a("div", { className: "flex items-center gap-1.5", children: [
                                /* @__PURE__ */ a("div", { className: "flex flex-col", children: [
                                  /* @__PURE__ */ t("button", { onClick: () => De(T, -1), disabled: T === 0, className: "text-[8px] leading-none disabled:opacity-30", style: { color: "var(--muted)" }, children: "▲" }),
                                  /* @__PURE__ */ t("button", { onClick: () => De(T, 1), disabled: T === F.length - 1, className: "text-[8px] leading-none disabled:opacity-30", style: { color: "var(--muted)" }, children: "▼" })
                                ] }),
                                /* @__PURE__ */ t(
                                  "input",
                                  {
                                ...
                                /* @__PURE__ */ t("button", { onClick: () => Ye(T), className: "text-[13px] leading-none px-1", style: { color: "var(--muted)" }, children: "×" })
                              ] }),
                              h.type === "agent" && /* @__PURE__ */ a("div", { className: "mt-1.5 pl-5 flex items-center gap-2 flex-wrap", children: [
                                ...
                                ((be = h.agent) == null ? void 0 : be.role) && /* @__PURE__ */ t("span", { className: "text-[10px] truncate", style: { color: "var(--muted)" }, children: h.agent.role })
                              ] }),
                                ...
                                /* @__PURE__ */ a(
                                  "select",
                                  {
                                    ...
                                    children: [
                                      ...
                                      lt.map((de) => /* @__PURE__ */ t("option", { value: de, children: de }, de))
                                    ]
                                  }
                                )
                              ] })
              ...
              ae === "webhook" && /* @__PURE__ */ t("div", { className: "px-5 py-4 overflow-y-auto flex-1", children: /* @__PURE__ */ t(Cr, {}) }),
              f && ae === "danger" && E && (() => {
                const h = b.includes("/") ? b.split("/")[1] : b, T = Le.trim() === h;
                return /* @__PURE__ */ t("div", { className: "px-5 pb-4 pt-4", children: v ? /* @__PURE__ */ a(
                  "div",
                  {
                  ...
                  "div",
                  {
                    ...
                    children: [
                      ...
                      /* @__PURE__ */ t(
                        ...
                        {
                          value: Le,
                          onChange: (ge) => at(ge.target.value),
                          placeholder: h,
              ...
              /* @__PURE__ */ a("div", { className: "px-5 py-3 flex justify-end gap-2", style: { borderTop: "1px solid var(--border)", background: "var(--bg-elevated, var(--card))" }, children: [
                /* @__PURE__ */ t("button", { onClick: i, className: "text-xs px-3 py-1.5 rounded-md font-medium", style: { color: "var(--muted)" }, children: ae === "settings" ? "Cancel" : "Close" }),
                ae === "settings" && /* @__PURE__ */ t(
                  "button",
                  {
                    disabled: !wt || !f && Nt,
                    onClick: () => p({
...
function Ga(e, r) {
  const [o, s] = w(""), l = je(""), p = je(""), u = je(null);
  return p.current = e || "", Oe(() => {
...
function Fa({ live: e }) {
  const [r, o] = w(!0), s = je(null), l = Ga(e.tail, e.active);
  Oe(() => {
  ...
  return /* @__PURE__ */ a("div", { className: "mt-2", style: { borderTop: "1px dashed var(--border)", paddingTop: "6px" }, children: [
    /* @__PURE__ */ a("div", { className: "flex items-center gap-1.5 text-[10px]", children: [
      ...
      e.active && /* @__PURE__ */ t(ft, { size: 10 }),
...
function Va({ card: e, events: r, children: o, parent: s, onOpenCard: l, onClose: p }) {
        ...
        "section",
        {
          ...
          children: [
            /* @__PURE__ */ a("header", { className: "px-5 py-3.5 flex items-start gap-3", style: { borderBottom: "1px solid var(--border)" }, children: [
              ...
              /* @__PURE__ */ t("button", { onClick: p, className: "text-[13px] px-2 py-0.5 rounded hover:opacity-80", style: { color: "var(--muted)" }, "aria-label": "Close", children: "✕" })
            ] }),
            /* @__PURE__ */ a("div", { className: "px-4 py-3 overflow-y-auto", children: [
              ...
              r.length === 0 ? /* @__PURE__ */ t("div", { className: "text-[12px]", style: { color: "var(--muted)" }, children: u ? "No events recorded on this card directly." : "No recorded events yet." }) : /* @__PURE__ */ t("ol", { className: "flex flex-col gap-2", children: r.map((i) => /* @__PURE__ */ a("li", { className: "flex gap-2 text-[12px]", children: [
                /* @__PURE__ */ t("span", { title: i.actor, "aria-hidden": "true", className: "flex-shrink-0 mt-0.5", children: Ha[i.actor] || "•" }),
                /* @__PURE__ */ a("div", { className: "min-w-0 flex-1", children: [
                  ...
                  /* @__PURE__ */ a("div", { className: "text-[9px] mt-0.5", style: { color: Ka[i.actor] || "var(--muted)" }, children: [
                    i.actor,
                    i.step ? ` · ${i.step}` : "",
...
function Qa() {
  const e = wr(), r = Er(), [o, s] = w([]), [l, p] = w([]), [u, i] = w({}), [c, g] = w(Ot), [v, E] = w(!0), [f, b] = w("pipeline"), [x, k] = w(/* @__PURE__ */ new Set()), [D, L] = w(!1), [A, I] = w(null), [C, B] = w([]), [X, y] = w([]), [J, S] = w([]), [he, ye] = w(!1), [le, me] = w(!1), [Se, Q] = w(!1), [z, ee] = w(!1), [$e, Te] = w(!1), [_e, _] = w(!1), [U, te] = w([]), we = je(null), qe = je(!1), ne = je(!1), Z = je(/* @__PURE__ */ new Set()), F = je(/* @__PURE__ */ new Set()), [ve, O] = w({}), ce = se(
    (n) => e.get("/api/file-read?path=" + encodeURIComponent(n)),
    [e]
  ), Le = se(async (n = !1) => {
    try {
      const d = !ne.current || n ? await Vt(ce) : await dr(ce, et);
      et = d.path, ne.current = !0;
      const m = d.data;
      s(m.cards || []), p(m.pipelines || []), i({ github_webhook_history: m.github_webhook_history || [], scheduler_state: m.scheduler_state || null }), g({ ...Ot, ...m.config || {} });
    } catch (d) {
        ...
        if (typeof G.data == "string")
          try {
            ...
            if (M.type === "slots" && Array.isArray(V)) {
              ...
              for (const q of V) {
                const Re = (q == null ? void 0 : q.key) || (q == null ? void 0 : q.slot) || (q == null ? void 0 : q.name), We = String((q == null ? void 0 : q.title) || (q == null ? void 0 : q.name) || "");
                typeof Re == "string" && Re.startsWith("cron-") && [...F.current].some((Ue) => We.includes(Ue)) && K.add(Re), typeof Re == "string" && (q != null && q.running) && K.has(Re) && Y.push(Re);
              }
              Z.current = K, Y.length && O((q) => {
                let Re = q;
                for (const We of Y) {
                  const Ue = sr(q[We]);
                  Ue !== q[We] && (Re = { ...Re, [We]: Ue });
  ...
  const it = 6e5, pe = Ce(() => {
    ...
    for (const j of ae) {
      ...
      }), oe = /* @__PURE__ */ new Set([...Object.keys(G), ...Object.keys(M)]);
      for (const K of oe) {
        const Y = G[K] || "idle", q = M[K], Re = Y === "pending" || Y === "error", We = !!(q != null && q.slot_key) && !q.chat_disabled_at && !q.superseded;
        if (!Re && !We) continue;
        const Ue = (d = j.pending_at) == null ? void 0 : d[K], re = Re && !!Ue && Date.now() - new Date(Ue).getTime() > it, ie = (m = V == null ? void 0 : V.steps) == null ? void 0 : m.find((Ge) => Ge.id === K), Be = (q == null ? void 0 : q.agent) || ((N = ie == null ? void 0 : ie.agent) == null ? void 0 : N.crew) || ((R = ie == null ? void 0 : ie.agent) == null ? void 0 : R.name) || "orchestrator", Ae = q == null ? void 0 : q.agent_id, ot = q == null ? void 0 : q.slot_key, Pt = q == null ? void 0 : q.session_key, Wt = Ae ? U.some((Ge) => Ge.id === Ae) : Re && U.some((Ge) => (Ge.task || "").includes(j.id) || (Ge.task || "").includes(j.title)), Ut = !!(q != null && q.last_response_at) && (!q.last_response_handled_at || q.last_response_handled_at < q.last_response_at);
        n.push({ cardId: j.id, card: j.title || j.id, step: K, agent: Be, stale: re, status: Y, live: Wt, responsePending: Ut, agentId: Ae, slotKey: ot, sessionKey: Pt, sessionName: q == null ? void 0 : q.name });
      }
    }
    return n;
  }, [ae, l, U]), $ = Ce(() => {
    ...
    const d = (n && n.length ? n : Yt).map((j) => ({ ...j })), m = new Set(d.map((j) => j.id)), N = [];
  ...
  }, [x, l]), W = Ce(() => $.map((n) => n.id), [$]), ke = se((n) => {
    var d;
    return ((d = $.find((m) => m.id === n)) == null ? void 0 : d.type) === "gate" || n.startsWith("gate-");
  }, [$]), Ye = se((n) => {
    var d, m;
    return ((m = (d = $.find((N) => N.id === n)) == null ? void 0 : d.agent) == null ? void 0 : m.name) || Ta[n] || "unknown";
  }, [$]), De = se((n) => {
    var R, j;
    ...
    if (!N) {
      const G = l.find((Y) => Y.id === n.pipeline_id) || l.find((Y) => {
        var q;
        return Y.repo === ((q = n.source) == null ? void 0 : q.repo);
      }), M = (j = G == null ? void 0 : G.steps) != null && j.length ? G.steps : Yt, V = [
        { id: "intake", name: "Intake", type: "agent" },
        ...M.filter((Y) => Y.id !== "intake" && Y.id !== "done"),
        { id: "done", name: "Done", type: "agent" }
      ], oe = V.findIndex((Y) => Y.id === n.stage), K = oe >= 0 ? V[oe] : void 0;
      if (N = K == null ? void 0 : K.reviews_step, !N && oe >= 0)
        for (let Y = oe - 1; Y >= 0; Y--) {
          const q = V[Y];
          if (!(q.id === "intake" || q.id === "done") && q.type !== "gate" && !q.id.startsWith("gate-")) {
            N = q.id;
            break;
          }
        }
  ...
  Oe(() => {
    const n = async () => {
      try {
        const N = et.slice(0, et.lastIndexOf("/")), R = (N ? N + "/" : "") + "live_spawns.json", j = await e.get("/api/file-read?path=" + encodeURIComponent(R));
        qe.current = !1;
  ...
  const Ie = se(async () => {
    ...
    try {
      ...
      n = Na(m == null ? void 0 : m.agents), y(n);
    } catch (m) {
      console.warn("crew roster (config.json) unreadable:", m), y([]);
    }
    const d = await Promise.all(_a(n).map(async (m) => {
      const N = Ca(m, n);
      if (!N) return tr(null, m);
      try {
        const R = await ce(N);
        return tr(R, m, N);
      } catch {
    ...
    try {
      const d = await Vt(ce);
      et = d.path, d.data.cards = d.data.cards || [], n(d.data);
      let m = d;
      try {
        m = await Vt(ce), et = m.path, m.data.cards = m.data.cards || [], n(m.data);
      } catch {
        m = d;
      }
    ...
    fe((G) => {
      ...
      m.type === "reject" && !oe || m.type === "interject" && !K || (M.gate_commands = M.gate_commands || [], M.gate_commands.some((q) => q.id === j) || M.gate_commands.push({
        id: j,
        gate: d,
        action: m.type,
    ...
    const N = (/* @__PURE__ */ new Date()).toISOString(), R = ga();
    fe((j) => {
      const G = j.cards.find((V) => V.id === n);
      if (!G) return;
      let M;
      try {
        M = xa({ id: R, kind: d, text: m, card: G, now: N });
      } catch {
        return;
      }
      G.interjection = fa(G.interjection, M), G.updated_at = N;
    });
  }, [fe]), _t = se((n) => {
    ...
    const m = ((G) => {
      ...
      const V = (oe = l.find((K) => K.id === (G == null ? void 0 : G.pipeline_id))) == null ? void 0 : oe.orchestrator_session;
    ...
    try {
      const G = await e.post("/apps/dlc-yolo/api/orchestrator/trigger", { card_id: n.id });
    ...
    fe((d) => {
      ...
      if (!m) return;
      const N = m.trust || ((R = d.config) == null ? void 0 : R.trust) || Ot.trust;
      m.trust = lt[(lt.indexOf(N) + 1) % lt.length], m.updated_at = (/* @__PURE__ */ new Date()).toISOString();
    });
  }, [fe]), qt = se((n) => {
    fe((d) => {
      var R;
      const m = d.cards.find((j) => j.id === n);
      if (!m) return;
      const N = m.depth || ((R = d.config) == null ? void 0 : R.depth) || Ot.depth;
    ...
    const n = Ie(), d = [];
    try {
      const m = await e.get("/api/file-read?path=~/.kiro/crew/config.json"), N = (m == null ? void 0 : m.workspaces) || {};
    ...
    }), L(!1), I(null), k(/* @__PURE__ */ new Set([n.repo]));
  }, [fe]), Mt = se(async (n) => {
    await fe((d) => {
    ...
    const n = Object.fromEntries(ur.map((d) => [d, []]));
    return ae.forEach((d) => {
      var j, G;
      ...
      n[vr(d, { isGate: N, liveObserved: R }).kind].push(d);
    }), Object.fromEntries(ur.filter((d) => n[d].length > 0).map((d) => [Sr[d].label, n[d]]));
  }, [ae, l, ke, pe]), mt = /* @__PURE__ */ new Set(["retired", "merged", "cancelled", "canceled", "superseded"]), h = ae.filter((n) => !mt.has(String(n.lifecycle || ""))).length, T = ae.filter((n) => ke(n.stage) && !mt.has(String(n.lifecycle || ""))).length, ge = ae.filter((n) => mt.has(String(n.lifecycle || ""))).length, be = ae.reduce((n, d) => {
    var m;
    ...
    var q, Re, We, Ue;
    const d = l.find((re) => re.id === n.pipeline_id) || l.find((re) => {
      var ie;
      return re.repo === ((ie = n.source) == null ? void 0 : ie.repo);
    }), m = ((Re = (q = d == null ? void 0 : d.steps) == null ? void 0 : q.find((re) => re.id === n.stage)) == null ? void 0 : Re.type) === "gate" || ke(n.stage), N = ["cancelled", "canceled", "retired", "merged", "superseded"].includes(String(n.lifecycle || "")), R = m && !N, j = R ? ((We = n.gate_review) == null ? void 0 : We.result_revision) ?? null : void 0, G = R ? De(n) : void 0, M = R ? He(n) : void 0, V = pe.some((re) => re.cardId === n.id && re.step === n.stage && re.live), oe = vr(n, { isGate: R, liveObserved: V }), K = (Ue = d == null ? void 0 : d.steps) == null ? void 0 : Ue.find((re) => re.id === n.stage), Y = n.capability || (K == null ? void 0 : K.capability) || "auto-derived";
    return {
      card: n,
      config: c,
      isGate: R,
      ...
      onReject: R ? (re) => wt(n.id, n.stage, { type: "reject", reason: re }, j) : void 0,
      onCycleTrust: () => er(n.id),
      onCycleDepth: () => qt(n.id),
      onSetBudget: (re) => Lt(n.id, re),
      onInterject: (re, ie) => wt(
        n.id,
        n.stage,
        { type: "interject", kind: re, text: ie },
        j
      ),
      onResolveDecision: (re) => nt(n.id, re),
      onOpenOrchestrator: () => Qt(n),
      liveView: (() => {
        var ot, Pt, Wt, Ut;
        const re = (Pt = (ot = n.step_sessions) == null ? void 0 : ot[n.stage]) == null ? void 0 : Pt.slot_key, ie = re ? ve[re] : void 0, Be = pe.some((Ge) => Ge.cardId === n.id && Ge.step === n.stage && Ge.live), Ae = zr((Wt = n.step_progress) == null ? void 0 : Wt[n.stage]);
        if (ie != null && ie.active)
          return {
            ...
            seq: ie.seq || 0,
            slotKey: re,
            onOpen: () => r(`/chat?sid=${encodeURIComponent(re)}`)
          };
        if (Ae && (Be || ((Ut = n.step_status) == null ? void 0 : Ut[n.stage]) === "pending"))
          return {
            stage: n.stage,
            phase: Ae.phase,
            tail: Ae.tail,
            ...
            slotKey: re || "",
            source: "progress-trail",
            onOpen: () => re && r(`/chat?sid=${encodeURIComponent(re)}`)
          };
      })(),
      allCards: ae,
      onRequest: (re, ie) => Nt(n.id, re, ie),
      onOpenStepSession: (() => {
        const re = n.step_sessions;
        if (!re || typeof re != "object") return;
        const ie = Object.entries(re).map(([Be, Ae]) => {
          const ot = (Ae == null ? void 0 : Ae.slot_key) || (Ae != null && Ae.session_key ? Ae.session_key.replace(/^cron:/, "cron-") : void 0);
          return ot ? { step: Be, open: () => r(`/chat?sid=${encodeURIComponent(ot)}`) } : null;
        }).filter((Be) => Be !== null);
        return ie.length ? ie : void 0;
      })(),
      onCancelCard: () => _t(n.id),
      onOpenCard: (re) => {
        const ie = document.getElementById(`card-${re}`);
        if (ie) {
          ie.scrollIntoView({ behavior: "smooth", block: "center" });
          const Be = ie.style.outline;
  ...
  return /* @__PURE__ */ a(Pe, { children: [
          ...
          "section",
          {
            ...
            children: [
              /* @__PURE__ */ a("header", { className: "px-5 py-3.5 flex items-center gap-3", style: { borderBottom: "1px solid var(--border)" }, children: [
                ...
                /* @__PURE__ */ t("button", { onClick: () => Te(!1), className: "text-[13px] px-2 py-0.5 rounded hover:opacity-80", style: { color: "var(--muted)" }, "aria-label": "Close", children: "✕" })
              ] }),
              /* @__PURE__ */ t("div", { className: "px-4 py-3 overflow-y-auto", children: /* @__PURE__ */ t(Ba, { cards: ae }) })
            ]
          }
        )
      }
        ...
        children: /* @__PURE__ */ a(
          "section",
          {
            ...
            children: [
              /* @__PURE__ */ a("header", { className: "flex items-start gap-4 px-5 py-4", style: { borderBottom: "1px solid var(--border)" }, children: [
                /* @__PURE__ */ a("div", { className: "min-w-0 flex-1", children: [
                  /* @__PURE__ */ a("div", { className: "flex items-center gap-2", children: [
                    ...
                    /* @__PURE__ */ t("span", { className: "text-[10px] font-semibold px-1.5 py-0.5 rounded-full", style: { background: "color-mix(in srgb, var(--accent) 14%, transparent)", color: "var(--accent)" }, children: pe.length })
                  ] }),
              ...
              /* @__PURE__ */ t("div", { className: "overflow-y-auto p-3 flex flex-col gap-2", children: pe.length === 0 ? /* @__PURE__ */ t("div", { className: "px-3 py-8 text-center text-[12px]", style: { color: "var(--muted)" }, children: "No linked agent chats yet." }) : pe.map((n) => {
                  ...
                  "div",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ a("div", { className: "flex items-center gap-2 text-[11px] min-w-0", children: [
                        ...
                        /* @__PURE__ */ t("span", { className: "font-semibold flex-shrink-0", style: { color: "var(--accent)" }, title: n.sessionName || void 0, children: n.agent }),
                        /* @__PURE__ */ a("span", { className: "truncate", style: { color: "var(--muted)" }, children: [
                          "· ",
                          n.step
                        ] }),
                        /* @__PURE__ */ t("span", { className: "ml-auto truncate max-w-[220px]", style: { color: "var(--text, var(--muted))" }, title: n.card, children: n.card }),
                        /* @__PURE__ */ t("span", { className: "flex-shrink-0", style: { color: n.responsePending ? "var(--warn)" : n.status === "pending" ? "var(--ok)" : "var(--muted)" }, children: n.responsePending ? "response" : n.status }),
                        n.stale && /* @__PURE__ */ t("span", { style: { color: "var(--warn)" }, title: "stale — will be reclaimed", children: "↻" })
                      ] }),
    ...
    A && /* @__PURE__ */ t(
      yr,
      {
        ...
        { id: "pl-" + A, repo: A, source: "manual", trust: c.trust, depth: c.depth, backlog_intake: !0, sot: "github", steps: Yt.map((n) => ({ ...n })), created_at: (/* @__PURE__ */ new Date()).toISOString() },
    ...
    /* @__PURE__ */ a("div", { className: "px-6 pb-8 overflow-y-auto flex-1 min-h-0", children: [
      /* @__PURE__ */ t(Ra, { steps: $, cardsByStage: dt, onNodeClick: Dt }),
      /* @__PURE__ */ a("div", { className: "grid gap-3 grid-cols-[repeat(auto-fit,minmax(120px,1fr))] mb-3", children: [
        /* @__PURE__ */ t(Gt, { label: "Active", value: String(h), accent: !0 }),
        /* @__PURE__ */ t(Gt, { label: "Gated", value: String(T) }),
        /* @__PURE__ */ t(Gt, { label: "Done", value: String(ge) }),
        /* @__PURE__ */ t(Gt, { label: "Parked", value: String(be) })
      ] }),
      /* @__PURE__ */ t(
        aa,
        {
          repos: at.map((n) => n.name),
          selectedRepos: [...x],
          onNewPipeline: () => {
            St();
          },
      ...
      /* @__PURE__ */ a("div", { className: "flex gap-4 items-start", children: [
        ...
        /* @__PURE__ */ a("div", { className: "flex-1 min-w-0", children: [
          /* @__PURE__ */ a("div", { className: "flex items-center gap-3 mb-4 flex-wrap", children: [
            /* @__PURE__ */ t(Aa, { active: f, onChange: b, counts: de }),
            /* @__PURE__ */ a(
              "button",
              {
                onClick: () => ee(!0),
            ...
            /* @__PURE__ */ a(
              "button",
              {
                ...
                style: { background: "var(--bg-elevated, var(--bg))", border: "1px solid var(--border)", color: Ke || zt || pe.some((n) => n.status === "pending" || n.responsePending) ? "var(--accent)" : "var(--muted)" },
                children: [
                  zt ? /* @__PURE__ */ t(ft, { size: 11 }) : /* @__PURE__ */ t(
                    "span",
                    {
                      className: Ke || pe.some((n) => n.status === "pending" || n.responsePending) ? "inline-block animate-pulse" : "inline-block",
                      style: { width: 7, height: 7, borderRadius: 999, background: Ke ? "var(--ok)" : pe.some((n) => n.responsePending) ? "var(--warn)" : pe.some((n) => n.status === "pending") ? "var(--accent)" : "var(--muted)", opacity: pe.length ? 1 : 0.5 }
                    }
                  ),
                  /* @__PURE__ */ t("span", { className: "font-semibold", children: pe.length ? `${pe.length} session${pe.length === 1 ? "" : "s"}` : "no sessions" }),
                  zt && /* @__PURE__ */ t("span", { children: "· thinking" }),
                  Ke && /* @__PURE__ */ t("span", { style: { color: "var(--ok)" }, children: "· generating" }),
                  !zt && !Ke && pe.filter((n) => n.status === "pending").length > 0 && /* @__PURE__ */ a("span", { children: [
                    "· ",
                    pe.filter((n) => n.status === "pending").length,
                    " running"
                  ] }),
                  pe.some((n) => n.responsePending) && /* @__PURE__ */ t("span", { style: { color: "var(--warn)" }, children: "· response" }),
          ...
          /* @__PURE__ */ t(Ia, { config: c, onSet: kt }),
          v ? /* @__PURE__ */ t("div", { className: "text-sm p-3", style: { color: "var(--muted)" }, children: "Loading pipeline…" }) : /* @__PURE__ */ a("div", { ref: we, className: "flex gap-3 overflow-x-auto pb-4", children: [
            f === "pipeline" && $.map((n) => /* @__PURE__ */ t(bt, { id: `stage-col-${n.id}`, title: n.name, count: (dt[n.id] || []).length, children: (dt[n.id] || []).map((d) => /* @__PURE__ */ t(gt, { ...ht(d) }, d.id)) }, n.id)),
            f === "pipeline" && pt.length > 0 && /* @__PURE__ */ t("div", { className: "flex-shrink-0 pl-3", style: { borderLeft: "2px dashed var(--border-strong, var(--border))" }, children: /* @__PURE__ */ t(bt, { id: "stage-col-done", title: "✅ Done", count: pt.length, children: pt.map((n) => /* @__PURE__ */ t(gt, { ...ht(n) }, n.id)) }) }),
            f === "pipeline" && ut.length > 0 && /* @__PURE__ */ t(bt, { id: "stage-col-cancelled", title: "⏹ Cancelled", count: ut.length, children: ut.map((n) => /* @__PURE__ */ t(gt, { ...ht(n) }, n.id)) }),
            f === "workspace" && Object.entries(Tt).map(([n, d]) => /* @__PURE__ */ t(bt, { title: n, count: d.length, children: d.map((m) => /* @__PURE__ */ t(gt, { ...ht(m) }, m.id)) }, n)),
            f === "crew" && Object.entries(Rt).map(([n, d]) => /* @__PURE__ */ t(bt, { title: n, count: d.length, children: d.map((m) => /* @__PURE__ */ t(gt, { ...ht(m) }, m.id)) }, n)),
            f === "status" && Object.entries(Bt).map(([n, d]) => /* @__PURE__ */ t(bt, { title: n, count: d.length, children: d.map((m) => /* @__PURE__ */ t(gt, { ...ht(m) }, m.id)) }, n))
          ] })
        ] })
      ] })
    ] })
